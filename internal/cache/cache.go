...
func (m *Manager) Get(key string) (model.ValidationResult, bool) {
	if val, found := m.cache.Get(key); found {
		if result, ok := val.(model.ValidationResult); ok {
			m.logger.Infof("License cached [expires: %d days | grace: %t]", 
				result.ExpiryDaysLeft, result.ActiveGracePeriod)
			return result, true
		}
	}
...
func (m *Manager) Store(key string, result model.ValidationResult) {
	// Store with a fixed TTL for security (ensure regular re-validation)
	m.cache.SetWithTTL(key, result, 1, 24*time.Hour)
